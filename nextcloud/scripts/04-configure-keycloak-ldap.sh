...
KEYCLOAK_LOCAL_URL="http://localhost:8081"
...
echo "[2/6] Creating new realm 'nextcloud'..."
${KCADM} create realms -s realm=nextcloud -s enabled=true -s displayName="Nextcloud SSO Realm" || echo "Realm may already exist"

echo "[3/6] Configuring LDAP user federation..."
# Get LDAP bind DN
LDAP_BIND_DN="uid=admin,cn=users,cn=accounts,dc=nextcloudauth,dc=lab"
LDAP_BIND_PASSWORD="${FREEIPA_ADMIN_PASSWORD}"
LDAP_USERS_DN="cn=users,cn=accounts,dc=nextcloudauth,dc=lab"
LDAP_GROUPS_DN="cn=groups,cn=accounts,dc=nextcloudauth,dc=lab"

${KCADM} create components -r nextcloud -s name=ldap-freeipa -s providerId=ldap -s providerType=org.keycloak.storage.UserStorageProvider \
  -s 'config.priority=["1"]' \
...
LDAP_ID=$(${KCADM} get components -r nextcloud --fields id,name | grep -B1 "ldap-freeipa" | grep "id" | cut -d'"' -f4)
...
${KCADM} create components -r nextcloud -s name=group-mapper -s providerId=group-ldap-mapper -s providerType=org.keycloak.storage.ldap.mappers.LDAPStorageMapper \
  -s "parentId=${LDAP_ID}" \
...
${KCADM} create user-storage/${LDAP_ID}/sync?action=triggerFullSync -r nextcloud
...
echo "  1. Go to: ${KEYCLOAK_URL}/admin/master/console/#/nextcloud"
