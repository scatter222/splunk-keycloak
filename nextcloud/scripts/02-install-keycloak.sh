...
KEYCLOAK_URL=http://${PUBLIC_IP}:8081
EOF
...
ExecStart=${KEYCLOAK_HOME}/bin/kc.sh start-dev --http-host=0.0.0.0 --http-port=8081
Restart=on-failure
RestartSec=10
...
echo "======================================"
echo "KeyCloak Admin Console: http://${PUBLIC_IP}:8081"
